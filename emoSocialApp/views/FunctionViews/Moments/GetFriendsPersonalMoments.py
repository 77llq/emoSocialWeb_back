...
class GetFriendsPersonalMomentsView(ModelViewSet):
    permission_classes = []

    def list(self, request, *args, **kwargs):

        user_id = request.query_params.get('friendId')
        user_id = User.objects.get(id=user_id)
        allMoments = Moments.objects.filter(postId=user_id).order_by('-postTime')
        momentsInfo = []
